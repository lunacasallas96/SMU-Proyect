DROP TABLE DESA_CLIENTES.RU.TMP_ARCHIVO_QA_M10 IF EXISTS;
CREATE TABLE DESA_CLIENTES.RU.TMP_ARCHIVO_QA_M10 AS
...
SELECT DISTINCT
	...
	, CASE WHEN NZ_CLIENTES.ADMIN.REPLACE(NZ_CLIENTES.ADMIN.REPLACE(UPPER(OF.DCTO),',','.'),'X','') > 100 
	THEN NULL
	ELSE (NZ_CLIENTES.ADMIN.REPLACE(NZ_CLIENTES.ADMIN.REPLACE(UPPER(OF.DCTO),',','.'),'X','')::FLOAT)::INTEGER||'%' 
	END AS "%Descuento"
	, NULL AS "Descuento Tarjeta"
	--, CASE WHEN NZ_CLIENTES.ADMIN.REPLACE(NZ_CLIENTES.ADMIN.REPLACE(UPPER(OF.DCTO),',','.'),'X','') > 100 THEN NZ_CLIENTES.ADMIN.REPLACE(UPPER(OF.DCTO),',','.')::INTEGER ELSE (AOP.MODAL*(1-NZ_CLIENTES.ADMIN.REPLACE(NZ_CLIENTES.ADMIN.REPLACE(UPPER(OF.DCTO),',','.'),'X','')::FLOAT))::INTEGER END AS"Precio Oferta por Factor"   ->> dejar el 24X cuando sean pack de unidades 
	, CASE 
		WHEN NZ_CLIENTES.ADMIN.REPLACE(NZ_CLIENTES.ADMIN.REPLACE(UPPER(OF.DCTO),',','.'),'X','')::INT > 100  and UPPER(OF.DCTO) NOT LIKE '%X%'
		then  (NZ_CLIENTES.ADMIN.REPLACE(NZ_CLIENTES.ADMIN.REPLACE(UPPER(OF.DCTO),'X',''), ',','.')::INT)::NVARCHAR(255)  
	 	when  NZ_CLIENTES.ADMIN.REPLACE(NZ_CLIENTES.ADMIN.REPLACE(UPPER(OF.DCTO),',','.'),'X','')::INT < 100  
		THEN (AOP.MODAL*('0.'||NZ_CLIENTES.ADMIN.REPLACE(OF.DCTO,'.0','')::INT)::FLOAT)::NVARCHAR(255)
		--THEN ((AOP.MODAL*(NZ_CLIENTES.ADMIN.REPLACE(NZ_CLIENTES.ADMIN.REPLACE(UPPER(OF.DCTO),',','.'),'X','')::FLOAT))::INTEGER)::NVARCHAR(255)
...
FROM --SELECT  AOP.MODAL,OF.DCTO ,CASE when  NZ_CLIENTES.ADMIN.REPLACE(NZ_CLIENTES.ADMIN.REPLACE(UPPER(OF.DCTO),',','.'),'X','')::INT < 100 THEN (NZ_CLIENTES.ADMIN.REPLACE(OF.DCTO,'.0','')::INT/100)::FLOAT END AS COLUMNA FROM
	DESA_CLIENTES.RU.APP_M10_OFERTAS OF
INNER JOIN --SELECT * FROM
	DESA_CLIENTES.RU.APP_M10_REL_IDS R
ON
	OF.OFFER_ID = CASE WHEN R.LISTAS_PRODUCTOS LIKE '-' THEN NULL ELSE SQLEXT..REPLACE(R.LISTAS_PRODUCTOS,'APP'||LPAD(OF.ORGANIZATION_ID,2,0)||LPAD(OF.ID_PERIODO,2,0)||LPAD($CICLO_P,3,0), '')::INTEGER END
...
--REGION OFERTA - EAN
		SELECT
			...
			, MODAL
		FROM
			DESA_CLIENTES.RU.APP_M10_OFERTAS_PLU AOP
		WHERE
			AOP.CYCLE = $CICLO
		--END REGION
	) AOP
...
DROP TABLE DESA_CLIENTES.RU.TMP_RANKING_EAN_M10 IF EXISTS;
CREATE TABLE DESA_CLIENTES.RU.TMP_RANKING_EAN_M10 AS
--REGION RANKING EAN
SELECT
	DISTINCT
	A.OFFER_ID
...
SELECT
	...
	, "CTA CTE" 
FROM --SELECT * FROM
	DESA_CLIENTES.RU.TMP_ARCHIVO_QA_M10 A
INNER JOIN--SELECT * FROM
	DESA_CLIENTES.RU.APP_M10_OFERTAS_PLU PLU
ON
	PLU.EAN = A.EANS
LEFT JOIN
	--SELECT * FROM
	DESA_CLIENTES.RU.TMP_RANKING_EAN_M10 B
ON
	A.EANS = B.EAN
	AND PLU.OFFER_ID  = B.OFFER_ID
WHERE
	PLU.CYCLE = $CICLO
	--AND PLU.OFFER_ID IN (6016,6373,6053,6060,6061,6083,6116)
;
--END REGION
