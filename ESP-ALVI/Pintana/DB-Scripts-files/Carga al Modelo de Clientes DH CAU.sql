...
SELECT
	...
	, 'APP DCTO CAU' AS CAMPAIGN_NAME
	, '2022-07-07 00:00:00'::DATE AS START_DATE
    , '2022-10-11 00:00:00'::DATE AS END_DATE
    --, B.FECHA_INICIO_PERIODO AS START_DATE
	--, B.FECHA_FIN_PERIODO AS END_DATE
	, 'UNI CAU' AS DESCRIPTION
...
CROSS JOIN
	(
		...
		SELECT
			DISTINCT
			'C'||$CICLO|| ' APP UNI CAU' AS CYCLE_DESCRIPTION
...
FROM
	(
		...
		SELECT
			...
			, CASE WHEN A.OFFER_ID <> 90000 THEN 'CRM_APP_CAU_' || A.OFFER_ID END AS OFFER_PRODUCT_LIST
			, 'APP' AS OFFER_PROMOTION_TYPE
			, A.TIPO_FINANCIAMIENTO||' CRM_APP_CAU'||A.CYCLE||'_'||A.OFFER_ID AS FINANCE_TYPE
		...
		FROM--SELECT * FROM
			...
			FROM
				DESA_CLIENTES.JL.APP_UNI_CAU OF
			) A
		INNER JOIN--SELECT  * FROM
			DESA_CLIENTES.JL.APP_UNI_REL_IDS_CAU B
		ON
...
WHERE
	CYCLE_ID = 1209 --ID_FUGADOS Y CAIDOS
	--CYCLE_ID IN (52,56,60,66,68,72)--PC
...
DROP TABLE DESA_CLIENTES.JL.TMP_BASE_CARGA_CUSTOMER_OFFER_CYCLE_CAU IF EXISTS;
CREATE TABLE DESA_CLIENTES.JL.TMP_BASE_CARGA_CUSTOMER_OFFER_CYCLE_CAU AS
...
WHERE
	A.LOAD_DATE = '2022-07-06 17:30:58'
...
--HASTA AQUI QUEDE CON EL DH MAL CARGADO
