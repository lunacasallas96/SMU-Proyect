...
INSERT INTO DESA_CLIENTES.JL.APP_TOTAL_BOLETA_CUENTAS_CORRIENTES_PINTANA
...
FROM
	...
	FROM--SELECT * FROM
		DESA_CLIENTES.JL.APP_ALVI_PINTANA A
	WHERE
		A.CYCLE = $CICLO 
	--END REGION
	) A
CROSS JOIN
	(
		...
		FROM
			DESA_CLIENTES.JL.APP_ALVI_PINTANA
		WHERE
			CYCLE = $CICLO
		--END REGION
	) B
...
SELECT 
	...
	, 1 AS MULTIPLICADOR
FROM--SELECT * FROM
	DESA_CLIENTES.JL.APP_TOTAL_BOLETA_CUENTAS_CORRIENTES_PINTANA A
INNER JOIN
	(
		...
		FROM
			DESA_CLIENTES.JL.APP_TOTAL_BOLETA_CUENTAS_CORRIENTES_PINTANA
		--END REGION
	) B
ON
...
FROM --SELECT * FROM
	DESA_CLIENTES.JL.STG_INPUT_BANDA_TOTAL_BOLETA_FUGADOS_ALVI_PINTANA A
INNER JOIN
	DESA_CLIENTES.JL.APP_ALVI_PINTANA B
ON
