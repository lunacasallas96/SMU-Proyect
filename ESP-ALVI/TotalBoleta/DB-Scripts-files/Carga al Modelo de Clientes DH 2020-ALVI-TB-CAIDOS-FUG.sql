...
SELECT
	...
	, 'FUGADOS Y CAIDOS ALVI PERIODO 9' AS CAMPAIGN_NAME
	...
	, 'FUGADOS Y CAIDOS ALVI PERIODO 9' AS DESCRIPTION
...
CROSS JOIN
	(
		...
		SELECT
			DISTINCT
			'C'||$CICLO|| ' APP BASKET TB FUGADOS' AS CYCLE_DESCRIPTION
...
CROSS JOIN
	(
		...
		SELECT
			DISTINCT
			'C'||$CICLO|| ' APP BASKET TB CAIDOS' AS CYCLE_DESCRIPTION
			, $CICLO AS CYCLE_NUMBER
			, A.FECHA_INICIO_CICLO AS START_DATE
			, A.FECHA_FIN_CICLO AS END_DATE
		FROM--SELECT * FROM
...
FROM
	(
		...
		FROM--SELECT * FROM
			...
			WHERE OF.CAMPAIGN LIKE '%FUGADOS%'
			) A
...
CROSS JOIN
	(
		...
		WHERE
			A.CAMPAIGN_ID = 113
			AND A.CYCLE_NUMBER = $CICLO
			AND A.CYCLE_ID IN ($c_caidos,$c_fugados)
		--END REGION
	) B
...
WHERE
	...
	AND B.CYCLE_ID IN ($c_caidos,$c_fugados)
...
WHERE
	CYCLE_ID = 1233 --ID_FUGADOS Y CAIDOS
	--CYCLE_ID IN (52,56,60,66,68,72)--PC
...
SELECT COUNT(1) FROM 
--DELETE FROM 
NZ_CLIENTES.IC.FACT_CUSTOMER_OFFER_CYCLE_DH 
WHERE CYCLE_ID = $C_FUGADOS
