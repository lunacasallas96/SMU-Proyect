...
SELECT DISTINCT
	...
	, CASE
	  ...
	  THEN
	  'S/F'||' CRM_ALVI_APP_'||UPPER(A.CICLO_COMERCIAL)||'_'||A.OFFER_ID 
	  WHEN UPPER(A.TIPO_FINANCIAMIENTO) = UPPER('sell out')
	  THEN
	  'S/O'||' CRM_ALVI_APP_'||UPPER(A.CICLO_COMERCIAL)||'_'||A.OFFER_ID 
	  ELSE
	  A.TIPO_FINANCIAMIENTO||' CRM_ALVI_APP_'||UPPER(A.CICLO_COMERCIAL)||'_'||A.OFFER_ID 
	  END
	  AS "Descripción"
...
FROM--SELECT * FROM
	(
		...
		FROM--select * from
			DESA_CLIENTES.NS.APP_ALVI_OFERTAS_PLU
		WHERE
			CYCLE = $CICLO
		--END REGION
	) AOP
INNER JOIN
	DESA_CLIENTES.NS.APP_ALVI_OFERTAS AO
ON
