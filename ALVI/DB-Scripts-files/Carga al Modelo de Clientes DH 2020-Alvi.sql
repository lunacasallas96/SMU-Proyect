...
SELECT
	...
	, 'APP ALVI PERIODO 13' AS CAMPAIGN_NAME
	, B.FECHA_INICIO_PERIODO::DATE AS START_DATE
	, B.FECHA_FIN_PERIODO::DATE AS END_DATE
	, 'CAMPAÑA APP ALVI PERIODO 13' AS DESCRIPTION
...
--END REGION
;
--USAR 125 PERIODO hASTA 2022-10-18 00:00:00
...
SELECT
	5 AS ORGANIZATION_ID
	, 125 AS CAMPAIGN_ID
	, 9 AS CAMPAIGN_TYPE_ID
	, MAX(CYCLE_ID) + 4 AS CYCLE_ID
	, B.CYCLE_NUMBER AS CYCLE_NUMBER
	, B.CYCLE_DESCRIPTION AS CYCLE_DESCRIPTION
	, B.START_DATE AS START_DATE
	, B.END_DATE AS END_DATE
...
CROSS JOIN
	(
		...
		SELECT
			DISTINCT
			'C'||CYCLE|| ' APP KIOSCO OFFERS' AS CYCLE_DESCRIPTION
			, CYCLE AS CYCLE_NUMBER
			, A.FECHA_INICIO_CICLO::DATE AS START_DATE
			, A.FECHA_FIN_CICLO::DATE AS END_DATE
		FROM--SELECT * FROM
		...
		WHERE
			A.CYCLE = $CICLO
			AND (UPPER(A.TIPO_OFERTA) LIKE UPPER('Kiosco%') OR UPPER(A.TIPO_OFERTA) LIKE UPPER('Estrella%'))
		--END REGION
	) B
GROUP BY 1,2,3,5,6,7,8,9
--END REGION
...
FROM
	(
		...
		INNER JOIN--SELECT * FROM
			DESA_CLIENTES.RU.APP_ALVI_REL_IDS B
		ON
...
FROM
	(
		...
		--	A.OFFER_ID = 90000
			A.CYCLE = $CICLO
			AND UPPER(A.TIPO_OFERTA) LIKE UPPER('Kiosco%')
		--END REGION
	) A
CROSS JOIN
	(
		...
		WHERE
			A.CAMPAIGN_ID = 125
			AND A.CYCLE_NUMBER = $CICLO
			AND A.CAMPAIGN_TYPE_ID = 9 --MASIVAS
		--END REGION
	) B
--END REGION
--END REGION
...
CROSS JOIN
	...
	--263786 
--END REGION
--multiplicar x 2 el resultado = 309896
--138346
/*SELECT
